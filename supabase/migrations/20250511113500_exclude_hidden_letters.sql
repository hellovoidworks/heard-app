-- Modify the get_unread_letters_not_by_user function to exclude hidden letters
CREATE OR REPLACE FUNCTION public.get_unread_letters_not_by_user(
  p_user_id uuid,
...
BEGIN
  ...
  WITH preferred_new_letters AS (
    ...
    WHERE
      ...
      -- Filter out letters from blocked users
      AND NOT EXISTS (
      ...
      -- NEW: Filter out hidden letters
      AND (l.hide IS NULL OR l.hide = FALSE)
    ORDER BY RANDOM()
    LIMIT p_limit
  )
