-- Update get_my_letters_with_stats to filter out reactions from bidirectionally blocked users
...
BEGIN
  ...
  LOOP
    ...
    -- Count replies for this specific letter, excluding blocked users AND reported threads
    ...
    -- Add report filtering logic
    AND NOT EXISTS (
      -- Check if the conversation with this participant has been reported
      SELECT 1
      FROM content_reports cr
      WHERE 
        cr.reporter_id = param_user_id 
        AND cr.letter_id = letter_record.id
        AND cr.content_type = 'reply'
        AND (
          -- Filter out if reported user is the author
          (cr.other_participant_id = r.author_id)
          OR 
          -- Also filter out if reported user is the recipient
          (cr.other_participant_id = r.reply_to_user_id)
          OR
          -- Or if the current user is replying to the reported user
          (r.author_id = param_user_id AND r.reply_to_user_id = cr.other_participant_id)
        )
    );
    
    -- Count reactions for this specific letter, excluding bidirectionally blocked users
    ...
    AND NOT EXISTS (
      -- Check if the requesting user is blocked by the reaction author (bidirectional blocking)
      SELECT 1 FROM user_blocks b
